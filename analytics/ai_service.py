...
ISSUE_EXAMPLES = {
    ...
    "FUNNEL_DROP": "Пример: критические точки отказа в воронке (аномальный отток на шаге).",
}
...
def analyze_issue_with_ai(issue_type, location, metrics_context):
    ...
    - Пример проявления (используй как ориентир, можно предлагать другие причины): {issue_hint}
...
def generate_stub_hypothesis(issue_type):
    ...
    stubs = {
        ...
        'FORM_ABANDON': "Гипотеза: Слишком много полей или ранний запрос чувствительных данных. Исправить: сократить форму, разбить на шаги с прогрессом и улучшить подсказки.",
        'FUNNEL_DROP': "Гипотеза: На этом шаге непонятен следующий шаг или внезапный барьер. Исправить: подсветить CTA/прогресс и убрать необязательные поля/шаги."
    }
